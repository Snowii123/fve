...
- **n-BMS** — fyzicky **Seplos** (potvrzeno uživatelem 13.8.2026; D-Bus pole `Manufacturer` hlásí "SHEnergy", což je jen identita CAN protokolu, kterou Seplos firmware reportuje — ne skutečný výrobce, viz [dbus-paths.md](../diagnostics/dbus-paths.md)). Komunikuje po CAN-bus přímo do Venus OS jako *managed battery* (Victron battery service). SOC, CCL (charge current limit) a DCL (discharge current limit) hlásí BMS, Victron/DVCC je jen konzumuje a řídí se jimi — SOC tedy **nepočítá Victron sám** (na rozdíl od systémů se SmartShunt/BMV, kde SOC počítá coulomb counting ve shuntu). Seplos má i vlastní oficiální konfigurační/monitorovací software (Battery Manager / BMS Studio) s per-článkovým live view a historií událostí — viz [dbus-paths.md](../diagnostics/dbus-paths.md#bms-je-seplos-ne-shenergy) pro detaily, zatím nepřipojeno.
...
✅ **Aktualizace (13.8.2026): `V_hard` už není odhad.** Přímým dotazem na n-BMS (root-dump, viz [dbus-paths.md](../diagnostics/dbus-paths.md)) potvrzeno `Info/MaxChargeVoltage` = **3,5625 V/článek** (57,0 V pack) — to je BMS-udávaný cíl/strop nabíjení, prakticky náš `V_hard`. RunVol 3,350 V má proti němu bezpečnou rezervu ~0,21 V/článek. Zároveň potvrzeno `Info/BatteryLowVoltage` = **2,9 V/článek** (46,4 V pack) jako dolní mez.
...
- ESS mód **"Keep batteries charged"** — pozorováno aktivní během incidentu 12.8.2026, aktivně tlačí pack k udržování 100 % SOC. Jde přímo proti plánované strategii "buffer pod stropem" ([fronius/README.md](../fronius/README.md)) — měl by se používat jen záměrně během řízených kalibračních cyklů, ne jako trvalý stav. Po incidentu přepnuto na **Optimized without BatteryLife**.
  - **Upřesnění (13.8.2026)**: samotný mód není problém — problém byl jeho kombinace s neomezeným proudem na nevyrovnaném packu. **S ručně nastaveným proudovým limitem 10 A** (viz [fronius/README.md](../fronius/README.md)) byl "Keep batteries charged" naopak použit záměrně a úspěšně jako nástroj k dotažení balancování (drží pack u stropu, kde Enerkey RunVol=3,350 V teprve má šanci pracovat) — bez proudového limitu zůstává rizikový.
