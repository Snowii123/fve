...
2. **Potvrzeno přímo v oficiálním Victron ESS manuálu (13.8.2026) — toto je pravděpodobně hlavní příčina.** Uživatel potvrdil, že na systému **je aktivní dodávka přebytku PV do sítě (feed-in)**, navíc s exportním stropem 9500 W nastaveným kvůli limitaci distributora. Manuál k tomu doslova píše: *"when enabling this option [Feed-in excess solar charger power], **the DVCC charge current limit configured under Settings → Limit charge current won't be active**. The solar charger will operate at full power for maximum feed-in into the grid."* — a rovnou varuje: *"configure a safe limit on the solar chargers when used with a **small battery bank**"* ([4. Configuration — Energy Storage System manual](https://www.victronenergy.com/media/pg/Energy_Storage_System/en/configuration.html)). To je přesně náš case: malý ~400 Ah pack, předimenzovaný 17,5kW Fronius, feed-in aktivní. Zdokumentovaná chybová hláška z komunity popisuje stejný mechanismus i jako obecnější ESS bug pro DC feed-in ([ESS - DC Feed-in enabled, Charge Current Limit (CCL) from BMS ignored](https://communityarchive.victronenergy.com/questions/106290/bug-in-ess-dc-feed-in-enabled-charge-current-limit.html)) — oficiální manuál mluví konkrétně o DC-coupled solar chargeru, náš Fronius je AC-coupled, ale stejný princip (feed-in = priorita exportu před respektováním proudového limitu) pravděpodobně platí analogicky i tady, a shoduje se s tím, že naše špička (41 A) nastala i při "Keep batteries charged" grid-nabíjení, ne jen při PV.
...
**Praktický důsledek — 10 A nastavené ve VRM tedy pravděpodobně vůbec není reálně vymáháno, dokud je feed-in aktivní.** Doporučení: dokud probíhá aktivní vyrovnávání packu, **dočasně vypnout/omezit dodávku přebytku do sítě** (feed-in) — podle manuálu by se tím DVCC limit charge current měl znovu reálně uplatnit. Bez toho nemá smysl jen sledovat a doufat — nastavené číslo v tuhle chvíli nic nezaručuje, jak ukázala i špička na 41 A.

**Otestováno živě (13.8.2026, 16:41–16:51 UTC) — feed-in vypnutý, jen částečně pomohlo.** Bezprostředně po vypnutí 4 čisté minuty (2-3 mV rozjezd, proud těsně u 10 A, 0 alarmů) — ale pak se objevil **stejný cyklický vzorec jako předtím**: ~4 min klid → ~3 min chaos (rozjezd až 814 mV, `max_cell_v` až 3,715 V, proud až 31 A, alarmy) → opakovat. Peak hodnoty byly o něco nižší než před vypnutím (31 A / 3,72 V vs. dřívějších 41 A / 3,84-3,87 V), ale **problém se nevyřešil, jen mírně zmírnil**. ➡️ Feed-in tedy pravděpodobně **přispívá**, ale **není jediná/hlavní příčina** cyklické nestability. Feed-in byl následně uživatelem vrácen zpět na zapnuto (standardní konfigurace systému kvůli exportu přebytků). Cyklické "hunting" chování (proud/CCL se během klidné fáze vrací nahoru, dokud znovu nenarazí na vychýlený článek) odpovídá spíš tomu, že **žádné statické VRM nastavení** (proudový strop ani feed-in) tenhle jev sám o sobě nezastaví — posiluje to případ pro rychlou externí automatizaci ([node-red-control-logic.md](../automation/node-red-control-logic.md)) místo spoléhání na nastavení a čekání.
