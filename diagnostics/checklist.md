...
- [ ] **Nová položka**: zjistit přesný název D-Bus battery service (`ssh root@<cerbo> "dbus -y | grep -i battery"`) — potřeba k použití [`scripts/poll-cerbo.sh`](scripts/poll-cerbo.sh) pro měření napěťové výchylky při proudové špičce (viz [fronius/README.md](../fronius/README.md)).
